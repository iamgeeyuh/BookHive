  ...
  useEffect(() => {
    const fetchAvailableSlots = async () => {
      try {
        ...
        console.log("Fetching reservation slots for room ID:", roomId, "and date:", localDateString);
        
        // Use the constructed localDateString for your API call to avoid timezone issues
        const response = await fetch(
          `${BACKEND_URL}/reservation?roomId=${roomId}&date=${localDateString}`,
          {
            method: 'GET',
            credentials: 'include',
          }
        ...
        reservations
          ...
          .forEach(reservation => {
            ...
            const reservationDate = startDate.toISOString().slice(0, 10);
            const selectedDateString = localDateString;
